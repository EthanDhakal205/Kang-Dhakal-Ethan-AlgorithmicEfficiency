...
class VisualizerPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (state.showCaptions && state.captionPhase != 0) {
            drawCaption(g2);
        }
    }

    private void drawCaption(Graphics2D g2) {
        if (state.captionText.isEmpty()) {
            return;
        }
    ...
    private void drawSearchArray(Graphics2D g2) {
        ...
        for (int row = 0; row < numRows; row++) {
            ...
            for (int index = startIndex; index < endIndex; index++) {
                ...
                int animationState = state.cellAnimState != null && index < state.cellAnimState.length
                    ? state.cellAnimState[index] : 0;

                int barHeight = allNumeric
                    ? minBarHeight + (int) (((((Number) state.currentArray[index]).doubleValue() - minValue) / range)
                        * (rowBarHeight - minBarHeight))
                    : rowBarHeight / 2;
                float boost = animationState == 2 ? 0.08f * animation : animationState == 3 ? 0.04f : 0f;
                int animatedBarHeight = (int) (barHeight * (1f + boost));
                int barY = baseY - animatedBarHeight;

                Color fill = animationState == 2 ? lerp(CELL_DEF, CELL_CHECK, animation)
                    : animationState == 3 ? lerp(CELL_CHECK, CELL_FOUND, animation)
                    : cellState == 1 ? CELL_SCAN : CELL_DEF;
                Color stroke = animationState == 2 ? lerp(BORDER, WARN, animation)
                    : animationState == 3 ? lerp(WARN, ACCENT2, animation)
                    : BORDER;

                if ((animationState == 2 || animationState == 3) && animation > 0.05f) {
                    Color glow = animationState == 3 ? ACCENT2 : WARN;
                ...
                g2.setColor(stroke);
                g2.setStroke(new BasicStroke((animationState == 2 || animationState == 3) ? 1.5f + animation * 0.5f : 0.5f));
                g2.drawRoundRect(x, barY, cellWidth, animatedBarHeight, 6, 6);

                if ((animationState == 2 || animationState == 3) && animation > 0.1f) {
                    Color cap = animationState == 3 ? ACCENT2 : WARN;
                ...
                drawArrayValue(g2, String.valueOf(state.currentArray[index]), x, barY, cellWidth, animatedBarHeight,
                    animationState == 2 || animationState == 3
                        ? lerp(TEXT, animationState == 3 ? ACCENT2 : WARN, animation)
                        : TEXT);
                drawArrayIndex(g2, index, x, cellWidth, baseY);

                if ((animationState == 2 || animationState == 3) && animation >= 0.1f) {
                    Color accent = animationState == 3 ? ACCENT2 : WARN;
            ...
            for (int index = startIndex; index < endIndex; index++) {
                ...
                if (locked) {
                    ...
                    switch (state.sortStepType) {
                        ...
                        case SWAP -> {
                            fill = new Color(16, 60, 42);
                            stroke = ACCENT2;
                            capColor = ACCENT2;
                            textColor = ACCENT2;
                        }
                        case MERGE, HIGHLIGHT -> {
                            fill = new Color(22, 36, 68);
                            stroke = ACCENT;
                            capColor = ACCENT;
    ...
    private void drawArrayValue(Graphics2D g2, String value, int x, int barY, int cellWidth, int barHeight, Color textColor) {
        g2.setFont(MONO_B);
    ...
    private void drawGraph(Graphics2D g2) {
        if (state.currentGraph == null || state.currentGraph.isEmpty()) {
            return;
        }
        int width = getWidth();
        ...
        for (Map.Entry<String, Point> entry : positions.entrySet()) {
            ...
            Color fill = onPath ? new Color(22, 68, 50)
                : current ? new Color(55, 40, 8)
                : visited ? new Color(18, 28, 52)
                : CARD;
            ...
            if (radius >= 12) {
                ...
                FontMetrics metrics = g2.getFontMetrics();
                g2.drawString(node, point.x - metrics.stringWidth(node) / 2, point.y + metrics.getAscent() / 2 - 1);
            }
        }
    ...
    private void drawString(Graphics2D g2) {
        ...
        if (windowStart >= 0 && patternLength > 0) {
            ...
            Color windowColor = state.strMismatch ? new Color(80, 30, 10) : new Color(20, 40, 80);
            g2.setColor(windowColor);
            g2.fillRoundRect(windowX, textRowY - 6, windowWidth, charHeight + 12, 10, 10);
            Color borderColor = state.strMismatch ? WARN : ACCENT;
        ...
        for (int position : state.matchPositions) {
            ...
            if (x + matchWidth < side || x > width - side) {
                continue;
            }
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (x + charWidth < side || x > width - side) {
                continue;
            }
            ...
            for (int position : state.matchPositions) {
                if (i >= position && i < position + patternLength) {
                    matched = true;
                    break;
                }
            }
            ...
            if (inWindow) {
                int positionInWindow = i - windowStart;
                if (state.strMismatch && positionInWindow == state.strMatchedChars) {
                    mismatch = true;
                } else if (positionInWindow < state.strMatchedChars) {
                    matched = true;
                }
            }

            Color fill = matched ? new Color(12, 65, 42)
                : mismatch ? new Color(80, 25, 10)
                : inWindow ? new Color(25, 38, 68)
                : CELL_DEF;
            Color textColor = matched ? ACCENT2
                : mismatch ? DANGER
                : inWindow ? new Color(160, 190, 255)
                : TEXT_DIM;
            ...
            g2.drawString(ch, x + (charWidth - metrics.stringWidth(ch)) / 2,
            ...
            g2.drawString(label, x + (charWidth - smallMetrics.stringWidth(label)) / 2, textRowY + charHeight + 14);
        }
    ...
    private void drawStringInfo(Graphics2D g2, String pattern, int y, int width) {
        ...
        if (state.strWindowStart >= 0) {
            String position = "window: [" + state.strWindowStart + ", " + (state.strWindowStart + pattern.length() - 1) + "]";
            ...
            g2.setColor(state.strMismatch ? DANGER : (state.strMatchedChars == pattern.length() ? ACCENT2 : TEXT_DIM));
    ...
    private Map<String, Point> treeLayout(int width, int height) {
        ...
        for (Map.Entry<Integer, List<String>> entry : nodesByLevel.entrySet()) {
            ...
            int y = topPadding + (numLevels <= 1 ? usableHeight / 2 : (int) ((double) level / (numLevels - 1) * usableHeight));
