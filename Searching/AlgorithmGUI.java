...
public class AlgorithmGUI extends JFrame {
    ...
    private int[]    cellStates;   // 0=default 1=scanned 2=checking 3=found
    private int      comparisons   = 0;
    private long     elapsedNs     = 0;
    private int      resultIndex   = -2; // -2 = not run yet
    private boolean  running       = false;
    private int      stepDelay     = 350;
    private String   statusMsg     = "configure and press run";
    ...
    private JPanel buildSidebar() {
        ...
        addCategory(listPanel, "ARRAY BASED", ARRAY_ALGOS);
        addCategory(listPanel, "GRAPH / TREE", GRAPH_ALGOS);
        addCategory(listPanel, "STRING / TEXT", STRING_ALGOS);
    ...
    private JPanel buildTopBar() {
        ...
        speedSlider.addChangeListener(e -> {
            ...
            stepDelay = delays[speedSlider.getValue() - 1];
        });
        speedPanel.add(speedLbl);
        speedPanel.add(speedSlider);
        bar.add(speedPanel, BorderLayout.EAST);
    ...
    private JPanel buildArrayInput() {
        ...
        JLabel arrLbl = dimLabel("array:");
        JTextField arrField = styledField(20);
        arrField.setText("2, 5, 8, 11, 14, 19, 27, 33, 45");
        arrField.putClientProperty("role", "array");

        JLabel tgtLbl = dimLabel("target:");
        JTextField tgtField = styledField(6);
        tgtField.setText("19");
        tgtField.putClientProperty("role", "target");

        JCheckBox icBox = new JCheckBox("ignore case");
        icBox.setFont(SMALL);
        icBox.setForeground(TEXT_DIM);
        icBox.setOpaque(false);
        icBox.setFocusPainted(false);

        arrField.addActionListener(e -> parseArrayInput(arrField.getText(), tgtField.getText(), icBox.isSelected()));
        tgtField.addActionListener(e -> parseArrayInput(arrField.getText(), tgtField.getText(), icBox.isSelected()));
        icBox.addActionListener(e -> parseArrayInput(arrField.getText(), tgtField.getText(), icBox.isSelected()));

        p.add(arrLbl); p.add(arrField);
        p.add(tgtLbl); p.add(tgtField);
        p.add(icBox);
        return p;
    }
    ...
    private JPanel buildGraphInput() {
        ...
        JLabel sLbl = dimLabel("start:");
        JTextField sField = styledField(4);
        sField.setText("A");

        JLabel tLbl = dimLabel("target:");
        JTextField tField = styledField(4);
        tField.setText("F");

        JLabel info = new JLabel("graph: A→B,C  B→D,E  C→F  D→∅  E→F  F→∅");
        info.setFont(SMALL);
        ...
        sField.addActionListener(gl);
        tField.addActionListener(gl);

        p.add(sLbl); p.add(sField);
        p.add(tLbl); p.add(tField);
        p.add(info);
        return p;
    }
    ...
    private JPanel buildStringInput() {
        ...
        JLabel txtLbl = dimLabel("text:");
        JTextField txtField = styledField(26);
        txtField.setText("the cat sat on the caterpillar");

        JLabel patLbl = dimLabel("pattern:");
        JTextField patField = styledField(10);
        patField.setText("cat");
        ...
        txtField.addActionListener(sl);
        patField.addActionListener(sl);
        icBox.addActionListener(sl);

        p.add(txtLbl); p.add(txtField);
        p.add(patLbl); p.add(patField);
        p.add(icBox);
        return p;
    }
    ...
    private JPanel buildBottomBar() {
        ...
        cmpLabel    = statLabel("comparisons", "—");
        timeLabel   = statLabel("time", "—");
        resultLabel = statLabel("result", "—");
    ...
    private void runBinary() throws InterruptedException {
        ...
        while (low <= high) {
            ...
            setCellState(mid, 1);
            if (c < 0) { markScanned(low, mid); low = mid + 1; }
            else        { markScanned(mid, high); high = mid - 1; }
        }
    ...
    private void runTernary() throws InterruptedException {
        ...
        while (low <= high) {
            ...
            setCellState(m1, 2); setCellState(m2, 2); sleep();
            comparisons += 2;
            ...
            if (compareValues(currentTarget, currentArray[m1]) < 0)      { markScanned(m1, high); high = m1 - 1; }
            else if (compareValues(currentTarget, currentArray[m2]) > 0) { markScanned(low, m2);  low  = m2 + 1; }
            else { markScanned(low, m1); markScanned(m2, high); low = m1 + 1; high = m2 - 1; }
        }
        resultIndex = -1; setStatus("not found");
    }
    ...
    private void runInterpolation() throws InterruptedException {
        ...
        while (low <= high) {
            ...
            if (currentArray[low] instanceof Number && currentTarget instanceof Number) {
                ...
                if (hi == lo) { pos = low; }
                else pos = low + (int) (((tv - lo) / (hi - lo)) * (high - low));
            } else { pos = (low + high) / 2; }
    ...
    private void runExponential() throws InterruptedException {
        ...
        if (matches(currentArray[0], currentTarget)) {
            setCellState(0, 3); resultIndex = 0; setStatus("found at index 0"); return;
        }
        setCellState(0, 1);
        int bound = 1;
    ...
    private void runGraphAlgo() throws InterruptedException {
        ...
        if (isBFS) { queue.add(graphStart); }
        else        { stack.push(graphStart); }
        ...
        while (isBFS ? !queue.isEmpty() : !stack.isEmpty()) {
            ...
            vizPanel.setGraphState(visited, node, Collections.emptySet(), null);
            sleep();
            ...
            if (node.equals(target)) {
                ...
                setStatus("found '" + target + "' — path: " + path);
                found = true;
                break;
            }

            List<String> neighbors = currentGraph.getOrDefault(node, new ArrayList<>());
        ...
        elapsedNs = 0;
        if (!found) { resultIndex = -1; setStatus("'" + target + "' not reachable"); }
        updateStats();
    }
    ...
    private void runStringAlgo() throws InterruptedException {
        ...
        if (selectedAlgo.equals("KMP Search")) {
            int[] lps = buildLPS(p);
            int i = 0, j = 0;
            while (i < n) {
                comparisons++;
            ...
            long BASE2 = 31, MOD2 = 1_000_000_007L;
            long ph = 0, wh = 0, pw = 1;
            for (int i = 0; i < m; i++) {
                ph = (ph * BASE2 + (p.charAt(i) - 'a' + 1)) % MOD2;
                wh = (wh * BASE2 + (t.charAt(i) - 'a' + 1)) % MOD2;
                if (i > 0) pw = (pw * BASE2) % MOD2;
            }
            comparisons++;
            if (wh == ph && t.substring(0, m).equals(p)) matchPositions.add(0);

            for (int i = 1; i <= n - m; i++) {
                wh = (wh - (t.charAt(i - 1) - 'a' + 1) * pw % MOD2 + MOD2) % MOD2;
                wh = (wh * BASE2 + (t.charAt(i + m - 1) - 'a' + 1)) % MOD2;
                comparisons++;
        ...
        setStatus(matchPositions.isEmpty() ? "no matches found"
                : matchPositions.size() + " match(es) at " + matchPositions);
        updateStats();
    }
    ...
    class VisualizerPanel extends JPanel {
        ...
        private String textStr = "", patStr = "";
        private int    textI = -1, patJ = -1;
        private List<Integer> strMatches = new ArrayList<>();
        private Set<String>   graphVisited = new HashSet<>();
        private String        graphCurrent = null;
        private Set<String>   graphPath    = new HashSet<>();
        private List<String>  graphPathList = null;
        ...
        void setMode(String m) { this.mode = m; repaint(); }
        ...
        private void drawArray(Graphics2D g2) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                g2.setColor(bg2);
                g2.fillRoundRect(x, startY, cellW, cellH, 8, 8);
                g2.setColor(border2);
                g2.setStroke(new BasicStroke(state > 1 ? 1.5f : 0.5f));
                g2.drawRoundRect(x, startY, cellW, cellH, 8, 8);
                ...
                g2.drawString(val, x + (cellW - fm.stringWidth(val)) / 2,
                              startY + cellH / 2 + fm.getAscent() / 2 - 4);
                ...
                g2.drawString(idx, x + (cellW - fm2.stringWidth(idx)) / 2,
                              startY + cellH + 16);
            }

            // pointer arrow for checking cell
            if (cellStates != null) {
                for (int i = 0; i < cellStates.length; i++) {
                    if (cellStates[i] == 2 || cellStates[i] == 3) {
                        int x = startX + i * (cellW + gap) + cellW / 2;
                        g2.setColor(cellStates[i] == 3 ? ACCENT2 : WARN);
                        g2.setStroke(new BasicStroke(2f));
                        g2.drawLine(x, startY - 8, x, startY - 24);
                        int[] px = {x - 5, x + 5, x};
                        int[] py = {startY - 22, startY - 22, startY - 8};
        ...
        private void drawGraph(Graphics2D g2) {
            ...
            for (Map.Entry<String, Point> e : pos.entrySet()) {
                ...
                Color fill   = isOnPath   ? new Color(30, 80, 60)
                             ...
                             : CARD;
                Color stroke2 = isOnPath  ? ACCENT2
                              : isCurrent ? WARN
                              : isStart   ? ACCENT
                              : isTarget  ? DANGER
                              : BORDER;
                ...
                FontMetrics fm = g2.getFontMetrics();
                g2.drawString(node, p.x - fm.stringWidth(node) / 2,
        ...
        private void drawString(Graphics2D g2) {
            ...
            int w = getWidth(), h = getHeight();
            int cellW = Math.min(22, (w - 48) / Math.max(textStr.length(), 1));
            int cellH = 36;
            int gap   = 3;
            int startX = 24;
            ...
            g2.setFont(SMALL);
            g2.setColor(TEXT_HINT);
            g2.drawString("text", startX, h / 2 - 30);
            ...
            for (int i = 0; i < textStr.length(); i++) {
                ...
                g2.setFont(new Font("JetBrains Mono", Font.PLAIN, Math.max(10, cellW - 6)));
                ...
                g2.drawString(ch, x + (cellW - fm.stringWidth(ch)) / 2,
                              ty + cellH / 2 + fm.getAscent() / 2 - 3);
            }

            // pattern row
            if (!patStr.isEmpty() && textI >= 0) {
                int py2 = ty + cellH + 24;
                g2.setFont(SMALL);
                g2.setColor(TEXT_HINT);
                g2.drawString("pattern", startX, py2 - 4);

                int alignX = startX + (textI - Math.min(patJ, textI)) * (cellW + gap);
                for (int j = 0; j < patStr.length(); j++) {
                    int x = alignX + j * (cellW + gap);
                    if (x < 0 || x > w) continue;
            ...
            if (!strMatches.isEmpty()) {
                g2.setFont(SMALL);
                g2.setColor(ACCENT2);
                g2.drawString(strMatches.size() + " match(es) found", startX, h - 16);
            }
        }
    }
    ...
    private void markScanned(int from, int to) {
        for (int i = from; i <= Math.min(to, cellStates.length - 1); i++) {
    ...
    private void updateStats() {
        SwingUtilities.invokeLater(() -> {
            ...
            if (resultIndex == -1) resultLabel.setText("result: not found");
            else if (resultIndex >= 0) resultLabel.setText("result: index " + resultIndex);
            vizPanel.repaint();
        });
    }
    ...
    private void parseArrayInput(String arrText, String tgtText, boolean ic) {
        try {
            ...
            currentArray = arr;
            try { currentTarget = Integer.parseInt(tgtText.trim()); }
            catch (NumberFormatException e1) {
                try { currentTarget = Double.parseDouble(tgtText.trim()); }
                catch (NumberFormatException e2) { currentTarget = tgtText.trim(); }
            }
            ignoreCase = ic;
            resetVisuals();
        } catch (Exception ex) { setStatus("invalid input"); }
    }
    ...
    private JLabel statLabel(String key, String val) {
